--This table contains sales conversions using product added as the event where the conversion event is within 1 day of the impression
{{
    config(
	incremental_strategy='delete+insert',
        unique_key='CONVERSION_ID'
    )
}}
WITH 
-- Create date filter for 2 days
{% if adapter.get_relation(this.database, this.schema, this.table) and not flags.FULL_REFRESH %}
T_MAX_DATA AS (select dateadd(HOUR, -48, max(PRODUCT_ADDED_AT)) AS MAX_DATA from {{ this }}), 
{% else %}
    T_MAX_DATA AS (select to_date('2018-01-01') AS MAX_DATA),
{% endif %} 

CAMPAIGN_DETAILS AS (
SELECT COUNTRY,
  CAMPAIGN_ID,
  CAMPAIGN_TYPE
FROM {{ ref('campaign_types')}} T
WHERE CAMPAIGN_TYPE = 'Sales'
),

BRAZE_EVENTS AS (
SELECT DISTINCT S.COUNTRY,
    S.CHANNEL,
    S.CAMPAIGN_ID,
    D.CAMPAIGN_TYPE,
    S.ID AS SEND_ID,
    EVENT,
    S.USER_ID,
    S.POC_ID,
    S.RECEIVED_AT AS IMPRESSION_TIME
FROM {{ ref('tracks_global_messages')}} S
JOIN CAMPAIGN_DETAILS D
    ON S.COUNTRY = D.COUNTRY
    AND S.CAMPAIGN_ID = D.CAMPAIGN_ID
{% if adapter.get_relation(this.database, this.schema, this.table) and not flags.FULL_REFRESH %}
--WHERE a.RECEIVED_AT > (select max(RECEIVED_AT) from {{ this }})
       WHERE RECEIVED_AT > (select MAX_DATA from T_MAX_DATA)
{% endif %}
),

SENDS AS (
SELECT DISTINCT COUNTRY,
    CHANNEL,
    CAMPAIGN_ID,
    CAMPAIGN_TYPE,
    SEND_ID,
    USER_ID,
    POC_ID,
    IMPRESSION_TIME
FROM BRAZE_EVENTS
WHERE EVENT IN ('in_app_message_viewed', 'push_notification_sent')
),

ADDS AS (
SELECT DISTINCT COUNTRY,
    PRODUCT_ADDED_ID,
    POC_ID,
    USER_ID,
    RECEIVED_AT AS PRODUCT_ADDED_AT,
    PRODUCT_SKU,
    QUANTITY_ADDED,
    BRAND,
    RECOMMENDATION_TYPE
FROM {{ ref('fact_global_product_added')}} 
{% if adapter.get_relation(this.database, this.schema, this.table) and not flags.FULL_REFRESH %}
--WHERE a.RECEIVED_AT > (select max(RECEIVED_AT) from {{ this }})
       WHERE RECEIVED_AT > (select MAX_DATA from T_MAX_DATA)
{% endif %}
)

SELECT DISTINCT A.COUNTRY,
    A.POC_ID,
    A.USER_ID,
    CAMPAIGN_ID,
    CHANNEL,
    SEND_ID,
    IMPRESSION_TIME,
    PRODUCT_ADDED_ID,
    PRODUCT_ADDED_AT,
    CONCAT(SEND_ID, PRODUCT_ADDED_ID) AS CONVERSION_ID,
    CURRENT_TIMESTAMP AS UPDATED_AT
FROM ADDS A
JOIN SENDS S
    ON A.COUNTRY = S.COUNTRY
    AND A.POC_ID = S.POC_ID
    AND A.USER_ID = S.USER_ID
WHERE DATEDIFF(SECONDS, IMPRESSION_TIME, PRODUCT_ADDED_AT) BETWEEN 0 AND 86400
