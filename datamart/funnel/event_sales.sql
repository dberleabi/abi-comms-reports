...
WHERE QUANTITY_ADDED > 0
    ...
    AND L_BRAND IN (SELECT DISTINCT BRAND FROM CAMPAIGN_DETAILS)
)
...
WHERE BRAND IN (SELECT DISTINCT BRAND FROM CAMPAIGN_DETAILS)
),
...
WHERE LOWER(BRAND_NAME) IN (SELECT DISTINCT BRAND FROM CAMPAIGN_DETAILS)
)
...
LEFT JOIN ORDER_ITEMS OI
    ...
    AND LOWER(BRAND) = LOWER(BRAND_NAME)
